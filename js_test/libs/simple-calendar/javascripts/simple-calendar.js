...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      for (var i = 0; i < 35; i++) {
        days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><div class="reminder-day"></div></div>';
      }
      //添加下拉框数据
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            if (data[year + '-' + currentmonth + '-' + day]) {
              ...
              var title = data[year + '-' + currentmonth + '-' + day];
              if(title === '请假'){
                v.querySelector('.reminder-day').innerHTML = `<span class='leave'>${title}</span>`;
              } else if (title === '出差') {
                v.querySelector('.reminder-day').innerHTML = `<span class='business-trip'>${title}</span>`;
              } else {
                v.querySelector('.reminder-day').innerHTML = `<span>${title}</span>`;
              }
            } else {
              //v.classList.remove('sc-mark');
              v.title = '';
              v.querySelector('.reminder-day').innerHTML = '';
            }
          });
        }
      }
